...
namespace CourseLibrary.API.Controllers
{
    ...
    public class AuthorsController : Controller
    {
        ...
        public IActionResult Create(AuthorForCreationDto authorForCreationDto)
        {
            if (ModelState.IsValid)
            {
                var authorEntity = new Entities.Author
                {
                    FirstName = authorForCreationDto.FirstName,
                    LastName = authorForCreationDto.LastName,
                    MainCategory = authorForCreationDto.MainCategory,
                };

                _courseLibraryRepository.AddAuthor(authorEntity);
                _courseLibraryRepository.Save();

                return RedirectToAction("Details", new { authorId = authorEntity.Id });
            }

            return View(authorForCreationDto);
        }
